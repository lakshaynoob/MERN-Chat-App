...
const UpdateGroupChat = ({ fetchAgain, setFetchAgain, fetchMessages }) => {
  ...
  const handleRemove = async (user1) => {
    ...
    try {
      ...
      //   if group admin removes himself from the group we don't want him to see the chats
      user1._id === user._id ? setSelectedChat() : setSelectedChat(data);

      setFetchAgain(!fetchAgain);

      //   reload messages after removing someone from the group
      fetchMessages();
  ...
  const handleAddUser = async (user1) => {
    ...
    try {
      ...
      toast({
        title: "Error Occured!",
        description: "error is uncontrollable",
        status: "error",
        duration: 5000,
        isClosable: true,
        position: "bottom",
